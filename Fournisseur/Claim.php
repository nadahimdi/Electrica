...
if(isset($_GET['live_search'])&&$_GET['live_search'] !== ""){
    ...
    $anomalieData = $crudObject->getRecDataSearch($filtervalues);
    $pages = 1;
}


elseif (!isset($_GET['live_search']) || $_GET['live_search'] === ""){
$anomalieData1 = $crudObject->getRecData();
...
$anomalieData = $crudObject->getRecDataFetch($startinglimit);}
?>
...
<!-- Anomalie MODIFY -->
<div class="modal fade" id="ClaimEditModal" tabindex="-1" aria-labelledby="ClaimEditModal" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="AnomalieAddModal">Manage Claims</h1>
      ...
      <input type="hidden" name="id" id="id">
        ...
        <input type="text" name="Respond"  id="Respond" class="form-control" required>
      </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
        <button type="submit" class="btn btn-danger">Respond</button>
      </div>
      </form>
    </div>
  </div>
...
<section id="content">
    ...
    <div class="Title">
    <h1 class="p fw-bold">Claims</h1></div>
    <nav>
    ...
    <img src="" alt="" id="popupImage">
        ...
        <h1 class="title p fw-bold">Claims</h1>
...
?>
    ...
    <td data-label='client_id'><?php echo $clientInfo[0]['CIN']; ?></td>
        ...
        <td data-label='Compteur'><?php echo $row['type']; ?></td>
        <td data-label='Compteur'><?php echo $row['contenue']; ?></td>
        <td data-label='Action'><button type="button" class="editClaimBtn"value="<?php echo $row['id']; ?>">Respond</button></td>
    </tr>
<?php
...
<script>
    ...
    if (res.status == 422) {
        ...
        $('#Respond').val(res.data.contenue_reponse);
        $('#id').val(res.data.id);
